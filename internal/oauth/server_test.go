...
func TestFullFlow(t *testing.T) {
	...
	authURL := ts.URL + "/oauth/authorize?" + url.Values{"response_type": {"code"}, "client_id": {clientID}, "redirect_uri": {"https://claude.ai/api/mcp/auth_callback"},
		"state": {"xyz"}, "code_challenge": {challenge}, "code_challenge_method": {"S256"}, "scope": {"kb"}}.Encode()
	...
	if check(access) != 200 || check("static") != 200 || check("kb_at_junk") != 401 || check("") != 401 {
		t.Fatalf("resource access: %d %d %d %d", check(access), check("static"), check("kb_at_junk"), check(""))
	}
...
func TestConfidentialClientAndBaseURL(t *testing.T) {
	...
	r.Header.Set("X-Forwarded-Host", "kb.example.com")
	if got := s.baseURL(r); got != "https://kb.example.com" {
