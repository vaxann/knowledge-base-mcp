...
- **Full-text search index** over notes (Cyrillic and Latin, stemming, frontmatter filters) with sub-100 ms queries, kept up to date incrementally; a "context bundle" tool returns the best-matching sections within a token budget for retrieval-augmented answers.
...
- **Write tools**: create (optionally from a template), replace, patch (append, replace section, set frontmatter), move/rename with link rewriting, delete (soft by default), all with optimistic concurrency.
- **Git versioning**: every mutation is exactly one atomic commit; automatic pull and debounced push; per-note history, diff and restore; conflicts never resolved by force.
...
- `search`: full-text search, filters, metadata queries, context bundles, index lifecycle and latency targets.
- `git-versioning`: commit-per-mutation, auto pull/push, conflict state, history/diff/restore, sync status.
...
- Non-goals for this change: semantic/vector search, multiple vaults per server, multi-user authorization, editing binary attachments, evaluating Dataview queries embedded in notes, an in-server LLM for "answering" (the calling model answers using search results).
