...
For clients that only speak OAuth (the Claude iOS/Android/web connectors), the binary embeds a minimal OAuth 2.1 authorization server for its own resource: metadata, dynamic client registration, PKCE authorization code flow with a password sign-in page, refresh rotation, state persisted as hashes in a JSON file on the index volume. Bringing an external identity provider would add a dependency for a single-user server; the password is the same trust boundary as the static token.
